...
export class NFTs {
    ...
    set(nfts: ResNFT[]) {
        const list = nfts.filter((nft) => nft.contract_type === "ERC721").map((nft, index) => {
        ...
        this.list = list
        list.map((nft) => {
            getImage(nft.uri).then((iconUrl) => this.setIconUrl(nft.address, nft.id, iconUrl))
        })
    }

    setIconUrl(address: string, id: string, iconUrl: string) {
        const index = this.list.findIndex((nft) => nft.id === id && nft.address === address)
